...
from db import execute_with_retry, now, get_db
...
def generate_error_hash(error_data: Dict) -> str:
    ...
    # Use error_type + error_message + url + method
...
def get_error_log_by_id(error_id: int) -> Optional[Dict]:
    ...
    try:
        ensure_error_table()
        cursor = execute_with_retry("SELECT * FROM error_logs WHERE id = ?", (error_id,))
        result = cursor.fetchone()
...
def get_error_log_by_request_id(request_id: str) -> Optional[Dict]:
    ...
    try:
        ensure_error_table()
        cursor = execute_with_retry("SELECT * FROM error_logs WHERE request_id = ? ORDER BY timestamp DESC LIMIT 1", (request_id,))
        result = cursor.fetchone()
        return dict(result) if result else None
    except Exception as e:
...
def clear_resolved_errors() -> int:
    ...
    try:
        ensure_error_table()
        cursor = execute_with_retry("DELETE FROM error_logs WHERE resolved = 1")
        conn = get_db()
