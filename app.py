...
def generate_csrf_if_needed():
    """
    Ensure a CSRF token exists for logged-in users.
    This runs before every request to guarantee the token is always available.
    """
    ...
    if request.method == 'POST':
        ...
        if student:
            if password == student['password']:
                ...
                # CSRF token is generated by ensure_csrf_token() in before_request
                session['csrf_token'] = secrets.token_hex(32)  # ensure it's set
...
def utility_processor():
    # Ensure CSRF token is available for logged-in users
